#
# modified from https://github.com/wjh892521292/LKM-UNet
# modified so that the LKM block is only in the bottleneck
# 
# 2p,2w
from re import S
...
class LKMUNetBot(nn.Module):
    def __init__(self,
        ...
        self.mamba = BiPixelMambaLayer(features_per_stage[-1], features_per_stage[-1])
        self.w_mamba = BiWindowMambaLayer(features_per_stage[-1],features_per_stage[-1])
        self.decoder = UNetResDecoder(self.encoder, num_classes, n_conv_per_stage_decoder, deep_supervision)

    def forward(self, x):
        skips = self.encoder(x)
        skips[-1] = self.mamba(skips[-1])
        skips[-1] = self.w_mamba(skips[-1])
