...
from dynamic_network_architectures.building_blocks.helper import maybe_convert_scalar_to_list, get_matching_pool_op, convert_conv_op_to_dim
...
class ResidualMambaEncoder(nn.Module):
    def __init__(self,
        ...
        for s in range(n_stages):
            feature_map_sizes.append([i // j for i, j in zip(feature_map_size, (strides[s], strides[s]))])
        ...
        for krnl in kernel_sizes:
            self.conv_pad_sizes.append([i // 2 for i in (krnl,krnl)])
...
class UNetResDecoder(nn.Module):
    def __init__(self,
        ...
        for s in range(1, n_stages_encoder):
            ...
            if s<3:
                ch = 2* input_features_below
            elif s==n_stages_encoder-1:
                ch = input_features_below//2
            else:
                ch=input_features_below
            ...
            attention_layers.append(Attention_block(F_g=input_features_skip, F_l=input_features_skip, F_int=ch))
        ...
        self.att_layers = nn.ModuleList(attention_layers)
    ...
    def forward(self, skips):
        ...
        for s in range(len(self.stages)):
            ...
            if s < (len(self.stages) - 1):
                x = self.att_layers[s](g=x, x=skips[-(s+2)])
