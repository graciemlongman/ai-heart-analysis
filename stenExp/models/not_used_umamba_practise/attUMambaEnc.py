...
class AttUMambaEnc(nn.Module):
    def __init__(self,
        ...
        super(AttUMambaEnc,self).__init__()

        self.Encoder = ResidualMambaEncoder(input_size=input_size,
            input_channels=3,
            n_stages = n_stages,
        ...
        self.Up_conv5 = conv_block(ch_in=1024, ch_out=256)

        self.Up4 = up_conv(ch_in=256, ch_out=256)
    ...
    def forward(self,x):
        ...
        d6 = self.Up6(x[-1]) #x[-1] = 512
        x5 = self.Att6(g=d6, x=x[-2]) #x[-2] = 512
        d6 = torch.cat((x5,d6), dim=1)
        d6 = self.Up_conv6(d6)

        d5 = self.Up5(d6)
        x4 = self.Att5(g=d5,x=x[-3]) #x[-3] = 512
        d5 = torch.cat((x4,d5),dim=1)       
        d5 = self.Up_conv5(d5)
        
        d4 = self.Up4(d5)
        x3 = self.Att4(g=d4,x=x[-4]) #x[-4] = 256
        d4 = torch.cat((x3,d4),dim=1)
        d4 = self.Up_conv4(d4)

        d3 = self.Up3(d4)
        x2 = self.Att3(g=d3,x=x[-5]) #x[-5] = 128
        d3 = torch.cat((x2,d3),dim=1)
        d3 = self.Up_conv3(d3)

        d2 = self.Up2(d3)
        x1 = self.Att2(g=d2,x=x[-6]) #x[-6] = 64
        d2 = torch.cat((x1,d2),dim=1)
        d2 = self.Up_conv2(d2)

        d1 = self.Up1(d2)
        x0 = self.Att1(g=d1, x=x[-7]) #x[-7] = 32
